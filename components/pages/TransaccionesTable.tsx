...
export function TransaccionesTable({ transactions, year, month }: TransaccionesTableProps) {
  ...
  function prevMonth() {
    ...
    router.push(`/transacciones?year=${d.getFullYear()}&month=${d.getMonth() + 1}`);
  }
  function nextMonth() {
    const d = new Date(year, month - 1, 1);
    d.setMonth(d.getMonth() + 1);
      ...
      <Card>
        <div className="flex items-center justify-between border-b border-border-subtle px-4 py-2.5">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-[140px] text-center text-xs font-semibold">
              ...
              <TableBody>
                {transactions.map((t) => (
                  <TableRow key={t.id}>
                    ...
                    <TableCell className="max-w-[280px] truncate text-fg-muted">
                      {t.note ?? '—'}
                    </TableCell>
                    <TableCell
                      className={
                        t.type === 'income'
                          ? 'text-right font-semibold tabular-num text-positive'
                          : 'text-right font-semibold tabular-num text-negative'
                      }
                    >
                      {t.type === 'income' ? '+' : '−'}
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Modificá los datos y guardá los cambios.</DialogDescription>
          </DialogHeader>
          {editing && categories ? (
            <TransactionForm
